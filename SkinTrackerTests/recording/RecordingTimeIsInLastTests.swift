...
class RecordingTimeIsInLastTests: XCTestCase {
    func testRecordingIsInLastRange() throws {
        ...
        let recordingTime = RecordingTime(recordingDate, .am)
        XCTAssert(recordingTime.isInLast(numberOfDays: 5, beforeDate: presentDate))
    }
    ...
    func testWorksForTwoWeekTimePeriod() throws {
        ...
        let recordingDate = Date(year: 2021, month: 7, day: 11, hour: 13, minute: 23)
        let recordingTime = RecordingTime(recordingDate, .am)
        XCTAssert(recordingTime.isInLast(numberOfDays: 14, beforeDate: presentDate))
    }
    ...
    func testExcludesTheFirstTooFarBackDayInTimeRange() throws {
        ...
        let recordingTime = RecordingTime(recordingDate, .pm)
        XCTAssert(!recordingTime.isInLast(numberOfDays: 14, beforeDate: presentDate))
    }
